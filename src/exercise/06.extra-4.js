...
function AppProvider({children}) {
  const reducer = React.useReducer(appReducer, {dogName: ''})
  return (
    <AppStateContext.Provider value={reducer}>
      {children}
...
function Cell({row, column}) {
  const [cell, setState] = useRecoilState(cellAtoms({row, column}))
  const handleClick = () => setState(Math.random() * 100)
